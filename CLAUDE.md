...
5. **Destructive-migration guard — never run a schema mutation against production without a same-boot restore point.** (See `INCIDENT_2026-07-18_CARD_SETS.md`: production card sets were lost with no backup to restore from.) Enforced in `start.sh`: a compressed `pg_dump` is written to the persistent volume (`/app/data/masked-cards/.db-backups/`, last 7 kept) before `drizzle-kit push` runs; if the dump fails, the push is skipped. Never remove this guard, never run `drizzle-kit push --force` (or any `DROP`/type-change) against production by hand without confirming a fresh dump exists, and never treat a `DELETE ... 0 rows` or `COUNT(*) = 0` result as verification — always confirm data state by reading back through the live app (`/api/...`) or the database the app's `DATABASE_URL` actually points to.
