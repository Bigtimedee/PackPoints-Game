...
Maker Rate on `GET /api/admin/metrics/making-layer` is `makers_30d / mau_30d` from `game_sets` and `event_log` (staff excluded). The same admin payload includes `publishedSetsNonStaff` + `makerSupplyGate` (lifetime count of non-staff `is_user_created` sets vs the diligence ≥10 gate) and `funnel.last7d` / `funnel.last30d` (MAKE_FLOW path + Design MAKE_FRICTION KPIs: identify fail rate, time-to-publish, name/mixtape drop-off, share open rate). Funnel events are `event_log` rows (`make_started`, `identify_*`, `name_started`, `publish_*`, `share_generated`, `share_opened`, `set_viewed`). No env vars required; definition lives in `server/services/makingLayerMetrics.ts`, `server/services/makingLayerEvents.ts`, and `ANALYTICS_DATA_DICTIONARY.md`. Maker Rate and funnel volume are admin-only — do not expose them on public surfaces.
