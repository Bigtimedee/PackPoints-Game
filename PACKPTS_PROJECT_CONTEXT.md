...
- [x] `/sets` cover kill switch (2026-09-26): `SETS_COVERS_DISABLED=1` or `true` makes `GET /api/sets` and `GET /api/sets/:id` return `coverCardUrls` / `previewCards` as `[]` and omit `shareImageUrl`, adds `coversDisabled: true`, and answers every `GET /api/sets/:setId/covers/:slot` with 404 and no-store (`private, no-store, no-cache, must-revalidate`, `CDN-Cache-Control: no-store`). The shelf tile keeps the 168px cover height and shows the set name, year, and Play this set, with no card count, image, gray box, or placeholder line. Unset, a ready cover is still `public, max-age=86400, stale-while-revalidate=604800`. Gameplay, masking, and deals are unchanged.
- [x] Cover QA export (2026-09-26): `COVER_QA_TOKEN` plus header `X-QA-Token` (constant-time, never a query param) opens `GET /api/qa/sets/:setId/cover-candidates` and `GET /api/qa/cover-image/:cardId`. Unset, blank, or a bad header is 404 with `Cache-Control: private, no-store, no-cache, must-revalidate` and `X-Robots-Tag: noindex`. Candidates are the same picker and filters as public covers (`n` default 12, max 24), so slots 0-7 are the current eight and later slots are the next spares. The image route returns that card's baked masked JPEG only, with `X-Card-Id` and `X-Mask-Version`, and refuses a blocked or unmasked card. These routes stay up when `SETS_COVERS_DISABLED` is on. A successful public cover also sends `X-Card-Id` and `X-Mask-Version`.
...
- `GET /api/qa/sets/:setId/cover-candidates` (header `X-QA-Token`) — Design review of the same cover picker. Off unless `COVER_QA_TOKEN` is non-empty. A missing or wrong header, or a token only in the query string, is 404 `{ error: "Not found" }`. `n` defaults to 12 and caps at 24. JSON is `{ setId, maskVersion, candidates: [{ slot, cardId, gameSetId, player, number, variant, maskVersion, bandPlacement, baked, imagePath }] }`. Order and filters match `GET /api/sets/:setId/covers/:slot` (deal eligibility, blocklist, sport match, baked sidecar, band guard, player dedupe), so slots 0-7 are the public eight. `bandPlacement` is `TOP_PLATE`, `BOTTOM_PLAQUE`, `PSA_SLAB`, or null. `baked` is true. `imagePath` is `/api/qa/cover-image/{cardId}`. Responses send `Cache-Control: private, no-store, no-cache, must-revalidate`, `CDN-Cache-Control: no-store`, `Surrogate-Control: no-store`, and `X-Robots-Tag: noindex`. This route ignores `SETS_COVERS_DISABLED`.
...
- `/sets` — play shelf for integrated sets already in PackPTS (`docs/SETS_POLISH.md`): eyebrow SETS, H1 “Sets”, sub “Play sets already in PackPTS.”, honest `{n} sets` from that list, Surface A cover crop or a fanned stack of baked masked cover URLs whose plaque is a solid bar with the gold seam and no label text. When `SETS_COVERS_DISABLED` is `1` or `true`, each tile keeps that cover height and shows only the set name, the year, and Play this set. No card count, image, gray box, or placeholder line. An empty cover keeps the cream card and that same bar and seam, with no PTS label and no photo URL. Integrated meta is `{n} cards` only (no `by Maker`, no date, no AUTHORED), blue **Play this set**, no short-shelf banner, empty state only at zero sets, Play Daily 5 (no Make a set). A stored brand missing from the set name is inserted into the heading (`2024 Basketball` + Topps → `2024 Topps Basketball`). No search chrome, no play counts, no Maker Rate, no playable-card vanity total. Also in mobile nav ("Browse") and home game-modes grid.
