...
**Last verified against codebase:** 2026-09-08 (SETS_POLISH authored dates = America/Chicago; staff QA identify-fail on `/make`)
...
- `/make` — 3-step wizard (Upload → Review → Publish) per MAKE_FLOW: auth-before-upload, sequential identify, name/mixtape → publish → Surface A share. No-draft step 1 uses Design EMPTY_STATE (5-card DESK example fan + Take photo / Choose from library; signed-out `Sign in to photo your stack.`). Draft rows use Design IDENTIFY_RETRY (`Identifying your stack`, Sequential board, Queued / Identifying… / Saved + green check / `Couldn't identify` + Try again + Skip). Staff/admin can force one Failed slot via `?qaIdentifyFail=1` / `?qa=identify-fail` (or one-shot storage) without calling identify; non-staff ignore the param. "Make it together" creates a collab session. After publish, default share preview is the runtime **I MADE THIS SET** PNG (that set’s masked cards + name + mixtape), not stock fan art. Contracts: `docs/design/EMPTY_STATE.md`, `docs/design/IDENTIFY_RETRY.md`, `docs/design/ENG_HANDOFF.md`. Funnel: `event_log` via `/api/make/event`.
- `/sets` — Goldin-quiet Maker sets shelf (`docs/SETS_POLISH.md`): eyebrow SETS, H1 “Maker sets”, sub “Built from the PC. Authored, not scrolled.”, Surface A cover crop or masked stack, `by {maker} · {n} cards · {MON D CT} · AUTHORED`, blue Play, short-shelf banner when published count is below 10, Make a set + Play Daily 5. No search chrome, no play counts, no Maker Rate. Also in mobile nav ("Browse") and home game-modes grid.
- `/sets/:id` — public set page: provenance, mixtape, honest card count + Play, optional “Play today’s stack” if not played today (CT), Surface A cover, THE STACK (masked, no player names), Share / Copy link / `packpts.com/sets/{slug}`. No Times Played tiles or currency chrome. Maker / co-creator get a quiet Save cover link for the Surface A PNG.
