...
- **Card blocklist** (`CARD_BLOCKLIST` in `server/lib/cardBlocklist.ts`) drops a card when `game_set_id` matches and the player field contains the listed name (case-insensitive). Current rows: 2024 Basketball (`229f0379` prefix) Antetokounmpo, 1987 Topps Football (`91cfdf3f-a620-4e73-adc8-22b8df221716`) Donnie Shell, 1989 Fleer (`aea515e2` prefix) Kevin Johnson. The predicate sits in `eligibleDealFilter` and in the Daily 5, 1v1, and replace queries that do not use that filter. Covers skip a blocked card and keep filling distinct players. A stored Daily 5 row for today that still points at a blocked card is rewritten to the next eligible card in that set before the hand or the masked image is served.
- **Oversized mask band** (`server/masking/maskBandLimit.ts`): if the union of full-width name bands (width at least 90%) is over 35% of the card height, the card is not served. A new bake skips the JPEG and writes the fail marker `mask_band_oversized`. `eligibleDealFilter` drops that `blocked_reason` even if `is_playable` is set true again. Already-baked v4.5 cards have no marker until the plan sidecar `{cardId}_v4.5.json` is read. That read happens on the serve, deal, and cover paths (in-memory cache) and in a production startup walk (`startOversizedBandScan`, batches of 20, concurrency 4) that writes the marker and sets `blocked_reason` without bumping `CURRENT_MASK_VERSION`. A missing plan is left for the next bake. No image is re-encoded. A registered profile whose full-width band is itself over 35%, including the 1987 Topps baseball bottom 46% fallback, fails closed the same way. Covers skip the card and keep filling distinct players. A stored Daily 5 row for today that still points at one of these cards is rewritten to the next eligible card before the hand or the masked image is served. A warm JPEG with a stale `.ok` sidecar is not sent.
