...
- [ ] Daily 5 card-1 hang “Finding a replacement card…” (2026-09-16): `GameCard.isPlaceholderImage` dominant-color >50% rejected a live HTTP 200 Topps Chrome JPEG; Daily 5 has no replace path so the overlay never clears. Same user then completed a separate 10-card solo with 1 skip after replace-card 404 (`sport unknown`). Audit: `docs/audits/DAILY5_STUCK_REPLACEMENT_2026-09-16.md`. Do not fix by wiring Daily 5 into solo `replace-card`.
