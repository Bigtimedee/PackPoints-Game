...
- [x] `/sets` cover kill switch (2026-09-26): `SETS_COVERS_DISABLED=1` or `true` makes `GET /api/sets` and `GET /api/sets/:id` return `coverCardUrls` / `previewCards` as `[]` and omit `shareImageUrl`, adds `coversDisabled: true`, and answers every `GET /api/sets/:setId/covers/:slot` with 404 and no-store (`private, no-store, no-cache, must-revalidate`, `CDN-Cache-Control: no-store`). The shelf then renders title, `{n} cards`, and Play this set, with no image and no placeholder line. Unset, a ready cover is still `public, max-age=86400, stale-while-revalidate=604800`. Gameplay, masking, and deals are unchanged.
