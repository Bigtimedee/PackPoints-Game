...
- **Card replacement:** If an image fails to load or is detected as a placeholder, the client requests a replacement card from the server (`POST /api/game/session/:id/replace-card` or WebSocket `question_replace_request`). Maximum 2 replacements before skipping. Solo `replace-card` writes the replacement onto that question index (so the opaque `/api/play/m/...` token for the index resolves to the new card). The solo GameCard mount key is `session + index + masked URL`, so a same-index replacement remounts; reveal keeps that key and only swaps `img src`. GameCard also clears `imageError` / `imageLoaded` when `imageUrl` changes. Landscape scans (aspect > 1.3) still trigger replacement — that rule is unchanged. Daily 5 does not call replace-card.
- **Post-submit full-card reveal (2026-09-16):** Dave’s rule: mask stays until a **successful** answer submit (correct or wrong), then the **full** card is shown; the next card starts masked. Guessing phase keeps the opaque bake (`/api/play/m/:scope/:sessionId/:index/:hmac?v=v4.4`) plus the GameCard overlay. After ACK, parents swap `img src` via `resolvePlayCardSrc` to the short-lived `/api/play/r/...` URL from that ACK and drop the overlay (`isRevealed`). `GET /api/images/card/:id` is not a public reveal URL. Solo sets `isRevealed` in submit `onSuccess` only (not on Submit click). Daily 5 `onSuccess`. 1v1 `answerResult !== null`. 1v1 replace uses `maskedCardImageUrl`. WS `sanitizeMatchStateForClient` sends `gameSetId` so overlay profiles match the set. `GameCard` stays dumb (renders the URL it is given). Do not weaken localization to fake a reveal. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`. **2026-09-24:** The reveal stays in that same in-flow slot (`data-testid="solo-card-slot"`). There is no reveal dialog and no click-to-enlarge. Revealed answer buttons stay `disabled` but use `disabled:opacity-100` so the correct/incorrect row is not painted at half opacity (that read as a scrim). Do not wrap the card in a modal on submit.
