...
- **Fairness:** Same cards for everyone; new-account detection (accounts < 7 days old may have restrictions). Daily 5 **never** calls solo `POST /api/game/session/:id/replace-card`. A name leak found after the deal is stored does not rewrite a past day. For today's ACTIVE or SCHEDULED row, if nobody has a registered entry or a guest run yet, each leaked position is replaced with the next eligible card in that set. If anyone has already started, the stored card stays, the masked JPEG is not served (422), and the client keeps the honest image error with no replace spinner.
...
- **Card blocklist** (`CARD_BLOCKLIST` in `server/lib/cardBlocklist.ts`) drops a card when `game_set_id` matches and the player field contains the listed name (case-insensitive). Current rows: 2024 Basketball (`229f0379` prefix) Antetokounmpo, 1987 Topps Football (`91cfdf3f-a620-4e73-adc8-22b8df221716`) Record Breakers Todd Christensen, Dave Jennings, Charlie Joiner, Steve Largent, Dan Marino, Donnie Shell, Phil Simms, plus Mark Duper and Charles Haley, 1989 Fleer (`aea515e2` prefix) Kevin Johnson. Name matches also drop those players' base cards in that set. The same 1987 set drops a card whose `number` normalizes (strip `#`, whitespace, leading zeros) to 2–8, and a card whose `player`, `variant`, or `description` matches `/record\s*breaker/i` (the letters RB do not drop a running back). Checklist sources: Cardboard Connection and Dean's Cards (cards 2–8, same seven names); PSA CardFacts states Record Breakers are cards 2–8. Mark Duper is base #236 and 1000 Yard Club #9 on Cardboard Connection, not a Record Breaker number, and stays name-blocked because the live cover showed his full name. Charles Haley is checklist #125 on that same set (Cardboard Connection lists `125 Charles Haley RC`; Sports Card Checklist lists `1987 Topps #125 Charles Haley`). His 49ers jersey is 94, which is Gerald McNeil's card on that checklist, so number 94 stays playable. The Super Rookie banner does not drop other cards. Haley in any other set stays playable. 1987 Topps baseball (`37fd025d`) also drops card id `36f1d909-2fdb-4b78-bb36-e7f5c088380a` (Mike Schmidt #28, jersey back reads HMIDT) and any card in that set whose normalized number is 28. Schmidt #430 stays playable. There is no name-wide Schmidt block in that set. 2024 Basketball (`229f0379`) drops the whole H insert subset: every card whose number matches `/^H-\d+$/i` (Stephen Curry H-3 is one). That pattern stays until a side-strip mask profile exists. BOD-5, ST-7, base numbers, a bare H, and HR-1 stay playable, and the pattern does not apply outside `229f0379`. Boot logs `[blocklist] set=37fd025d blockedIds=1 blockedNumbers=1 blockedPatterns=0` and `[blocklist] set=229f0379 blockedIds=0 blockedNumbers=0 blockedPatterns=1`. Boot logs `[blocklist] set=91cfdf3f recordBreakerNumbers=2,3,4,5,6,7,8 haleyNumber=125 players=Todd Christensen,Dave Jennings,Charlie Joiner,Steve Largent,Dan Marino,Donnie Shell,Phil Simms,Mark Duper,Charles Haley`. Multi-player cards are excluded on every set by the same predicate. Text on `player`, `variant`, or `description` drops a card on its own for Team Leaders, League Leaders, Leaders, Checklist, Combo/Combos, Tandem/Tandems, Duo/Duos, Trio/Trios, and `vs.` with the period. Bare `VS` does not. Super Bowl, All-Star, All Star, Future Stars, Rookie Stars, Prospects, and Highlights do not drop a card by themselves, including a single-player 1989 Topps All-Star and a single-player 2024 Basketball insert; those cards drop only when the player field names more than one person or the normalized number is on a vintage list. Record Breaker text stays limited to 1987 Topps Football and does not match a position abbreviation RB. A player field with `/`, ` & `, ` and `, or ` vs ` between names is multi-player. ` - ` splits two people only when both sides look like names and neither side is a subset or position label, so `Tony Gwynn - All-Star` stays eligible. A comma blocks two full names (`Karl Malone, John Stockton`) and three names. `Smith, John Paul`, `Last, First`, suffixes (`Jr.`, `Sr.`, `III`), and hyphenated surnames stay eligible. 1989-90 Fleer (`aea515e2`) also drops All-Star sticker numbers 1-11 (Isiah Thomas sticker 6, Chris Mullin sticker 9). Base cards 50 and 55 stay playable. 1987 Topps baseball (`37fd025d`) drops Mark McGwire number 366 only. Vintage multi-player numbers (team cards, league leaders, checklists, All-Star combos) are blocked by normalized `number` for `aea515e2`, `37fd025d`, `352b33d1`, `a09b2fe7`, and `91cfdf3f`. Boot logs `[blocklist] set=<8> multiPlayerNumbers=<list> textRules=on` for each of those sets. The predicate sits in `eligibleDealFilter` and in the Daily 5, 1v1, and replace queries that do not use that filter. Covers skip a blocked card and keep filling distinct players. The initial Daily 5 draw (`selectCardsForChallenge`, called from `createChallengeForDate` the first time a CT date is requested) uses `cardNotBlockedSql`, the same predicate inside `eligibleDealFilter`. A stored `daily_challenges` row for today or any later date that still points at a blocked card is rewritten at serve time (`sweepBlockedDaily5Deals` from `getOrCreateTodayChallenge`) to the next eligible card. The log is `[Daily5] blocklist swapped date=<YYYY-MM-DD> slot=<n> old=<card id> new=<card id>`. The same sweep rewrites a card the current mask version refused (`{cardId}_v4.6.fail`, including `name_plate_unresolved`, or a mask `blocked_reason` that `maskNameStillCovered` already excludes). The log is `[Daily5] mask refusal swapped date=<YYYY-MM-DD> slot=<n> old=<card id> new=<card id> reason=<reason>`. `runCardPoolRefreshJob` leaves that card non-playable (`cardPoolRefreshCandidateFilter` and `restorePlayableIfMaskAllows` in `server/services/cardPoolRefresh.ts`, both via `refusedAtCurrentMask`); it does not bake. A bake that clears the current-version fail sidecar is what makes the card dealable again. A past date is not rewritten. There is no in-memory hand cache and no precomputed future hand; the row is created on the first status or start request for that date.
- **Mask band guard** (`server/masking/maskBandLimit.ts`, env `MASK_BAND_GUARD`): `report` (default) or `enforce`. Placement is the edge the baker painted on that card, not the set profile. A full-width band (width at least 90%) that starts within 3 percentage points of the top is a top plate and fails above 35% of card height. One that ends within 3 points of the bottom is a bottom plaque and fails above 55%. The 3-point window sits above the baker's 2.5% pad floor in `fitNamePlateBand` and inside its 4% edge snap, so a real bake that sits on the card edge passes, and a band that starts 19% down and runs 77% (ends at 96%) fails as `mask_band_misplaced`. A bottom band at 46% passes. A top band at 36% or a bottom band at 56% would be `mask_band_oversized`. Each full-width band is checked on its own against the edge it touches, including a card masked at both the top and the bottom. The scan reads `{cardId}_v4.6.json` (the current mask version) on boot and does not read a leftover v4.5 plan. Report mode (`startMaskBandGuardScan`, batches of 20, concurrency 4) logs `[mask-band-guard] set=<8-char id> title=<set name> eligibleNow=<n> wouldDrop=<n> oversized=<n> misplaced=<n> eligibleAfter=<n>` plus `set=all title=all` for the total. It writes nothing and excludes nothing. Enforce mode writes the fail marker, drops `.ok`, sets `blocked_reason`, and `eligibleDealFilter` drops both reasons. Covers fall through. A stored Daily 5 card for today is swapped before it is served. The player-name blocklist is not behind this flag. `CURRENT_MASK_VERSION` is `v4.6`. Production sets `MASK_BAND_GUARD=enforce`. The code default stays `report` when that variable is unset. No image is re-encoded.
...
- **Card rows:** `GET /api/playable-sets/:id/cards`, `GET /api/cards`, `GET /api/cards/:cardhedgeCardId`, `GET /api/card-sets/:id/cards`, and `GET /api/diag/card-review-test` are admin-only (`isAuthenticated` + `requireAdmin` in `server/services/lockedCardRows.ts`). An admin receives the full row, including blocklisted cards and raw scan URLs. Logged-out is 401 and a non-admin is 403. `GET /api/cards/stats` stays a public count. `POST /api/onboarding/start` returns `maskedCardImageUrl` only. Public set pages keep using `GET /api/sets` and `GET /api/sets/:id`.
...
| `SETS_COVERS_DISABLED` | off | `1` or `true` hides `/sets` cover URLs and the cover image route. Any other value, including unset, leaves covers unchanged |
| `COVER_QA_TOKEN` | unset | When set, header `X-QA-Token` opens the Design cover review routes. Unset or blank is 404. Never accepted as a query param |
...
- [x] Pinned /sets covers (2026-09-26): `server/config/pinnedCovers.ts` maps `gameSetId` to `{ set, picks, alternates }`, changed only by PR. The public shelf and cover route walk picks, then alternates, and keep the first cards that pass `isBlockedCard`, the band guard, `eligibleDealFilter` plus sport match, and a baked masked JPEG, up to 8. A failure is skipped. Nothing outside the list fills a slot. Zero valid cards matches the hidden state, and that state does not render a cover slot. `SETS_COVERS_DISABLED` still forces that hidden state. Boot logs `[PinnedCovers] set=<id> picks=N alternates=M valid=K skipped=...`. QA `cover-candidates` marks each card `pick`, `alternate`, or `skipped` with the reason. A short fan (1 to 8) spreads the real covers at the normal cover height. The set detail page with no covers shows the name, the year label, and Play, with no placeholder, gray box, card count, or coming soon.
- [x] Set display override (2026-09-26): `shared/setDisplayOverride.ts` keys `229f0379-aa56-40a8-abe3-1af217a397e8` to title `2024-25 Topps Chrome Basketball` and year label `2024-25` (hyphen, not an en dash). Players see that on the /sets tile, the set detail page, Daily 5 when that set is the challenge, solo share and plaque, the set picker, the lobby, the home set-of-week line, marketplace set labels, and play-sets share/OG. Stored `set_name` and `year` are unchanged.
- [x] `/sets` chat clearance (2026-09-26): at 320 to 430px, `/sets` and `/sets/:id` end the scrollport at the top of the fixed feedback button (`bottom-20` + `h-12`, plus `safe-area-inset-bottom`) and pad the page by that button height plus `safe-area-inset-bottom`. Play this set, Play Daily 5, Share, Copy link, and Go home scroll fully clear and are not covered while scrolling. The button stays on screen. Cover tiles and the 1280 layout stay as signed off. Gameplay, masking, and `/api` routes are unchanged.
- [x] `/sets` cover kill switch (2026-09-26): `SETS_COVERS_DISABLED=1` or `true` makes `GET /api/sets` and `GET /api/sets/:id` return `coverCardUrls` / `previewCards` as `[]` and omit `shareImageUrl`, adds `coversDisabled: true`, and answers every `GET /api/sets/:setId/covers/:slot` with 404 and no-store (`private, no-store, no-cache, must-revalidate`, `CDN-Cache-Control: no-store`). The shelf tile and the set detail page keep a clean hidden state: set name, year label, and Play, with no cover slot, card count, image, gray box, or placeholder line. Unset, a valid pinned cover is still `public, max-age=86400, stale-while-revalidate=604800`. Gameplay, masking, and deals are unchanged.
- [x] Cover QA export (2026-09-26): `COVER_QA_TOKEN` plus header `X-QA-Token` (constant-time, never a query param) opens `GET /api/qa/sets/:setId/cover-candidates` and `GET /api/qa/cover-image/:cardId`. Unset, blank, or a bad header is 404 with `Cache-Control: private, no-store, no-cache, must-revalidate` and `X-Robots-Tag: noindex`. `candidates` are the valid pins in order (what the shelf serves). `picker` is the old auto order. `pins` is each configured id with `served` or `dropped` and a reason. `pinnedCount`, `validCount`, and `coversDisabled` are on the same JSON. The image route returns that card's baked masked JPEG only, with `X-Card-Id` and `X-Mask-Version`, and refuses a blocked or unmasked card. These routes stay up when `SETS_COVERS_DISABLED` is on. A successful public cover also sends `X-Card-Id` and `X-Mask-Version`.
- [x] Dealable-card QA sweep (2026-09-26): the same token opens `GET /api/qa/sets` and `GET /api/qa/sets/:setId/dealable-cards`. Unset, blank, missing, or wrong `X-QA-Token` is 404. A token in the query string is ignored. The set list is active integrated sets (`is_active`, not user-created) with a total from `eligibleDealFilter` (including current-mask fail sidecars). The card page is that same filter, ordered by `number` then `cardId`, `offset` default 0, `limit` default 200 and max 500. Each card is `{ cardId, player, number, variant, bandPlacement, baked }`. No new table.
- [x] Mask-bake refusal log (2026-09-26): every v4.6 bake that refuses (coverage or enforced band) inserts `mask_bake_refusals` with card id, set id, reason, layout class, profile source, the expected profile plate in source pixels, OCR boxes (text and confidence), candidate plates and why each failed, paint regions, mask version, timestamp, and the upright source bytes. `GET /api/qa/rejected-cards` lists those rows (no image bytes). `debug.png` draws the expected plate, candidates, and OCR boxes. `source` is the raw upright scan. `attempt.png` is the dry-run paint of the stored regions. Same token gate and no-store headers. Those images are not written to the masked-card cache and are not linked from player routes. `name_plate_unresolved` stays fail-closed. `POST /api/qa/rejected-cards/backfill` dry-runs that same resolver for cards that already have a current-mask fail sidecar or a mask `blocked_reason`, and inserts a `mask_bake_refusals` row when that card has no v4.6 row yet. Pinned cover ids go first. A dry-run that would pass is stored as `resolves_now` and does not clear the sidecar, `blocked_reason`, or deal eligibility. It does not write a masked JPEG, a sidecar, or `card_image_mask_cache`. It holds one warm bake slot with the same deadline as a player bake.
- [x] `/sets` index polish (2026-09-25): fanned thumbs paint a solid plaque with the gold seam and no "WHO IS THIS PLAYER?" label. The short-shelf banner is removed (it undersold a 7-set shelf). Index titles insert a stored brand the set name omitted, unless `shared/setDisplayOverride.ts` replaces the title. The year column is not written into the database. In-game plaque, mask bake, and reveal are unchanged.
...
- [x] First-session onboarding tutorial (Prompt 17): user_onboarding table; GET /api/onboarding/status, POST /api/onboarding/start (masked image URL only: no player, number, variant, or raw scan), POST /api/onboarding/complete (marks done, awards 50 PackPTS via idempotency key `onboarding_reward_${userId}`, returns nextAction hint); migration applied to prod
...
- `GET /api/sets` (public) — the `/sets` shelf of **integrated** sets: `is_active = true` AND `is_user_created = false`, at least 5 eligible cards, deduped by name/year/sport (keep the row with the most eligible cards). Ordered by `created_at` DESC after that filter. User-created sets are never listed (UGC publishing is closed). `cardCount` is the eligible deal count, the same predicate as `GET /api/playable-sets` and `getRandomCardsFromSet` (`server/services/playableSetEligibility.ts`): playable, `content_verified` null or true, https image, non-empty player, category matches the set sport, plus the deal exclusions (rejected review, known silhouette URLs, quarantined-unplayable). Each row keeps the previous shape: `cardCount`, `shareImageUrl` (Surface A `content_assets` lookup; stock fan `maker-set-1080.png` stripped), `coverCardUrls` (up to 8 `/api/sets/{setId}/covers/{slot}` URLs for the valid pins in `server/config/pinnedCovers.ts`, in that order; a pin that fails drops and nothing fills it; empty when the list is empty or no pin passes, and the client then shows the hidden slot with no cream card, gray box, or card count; no raw image URL, player name, or card id; the cover route does not bake), `makerUsername`, `playCount`, and `createdAt` as ISO UTC. `playCount` remains on the JSON; public `/sets` UI must not render it. Integrated rows do not render a maker, a date, or AUTHORED. If a solo start asks for more questions than that set has eligible cards, the session deals the eligible stack and sets `totalQuestions` to the dealt length. Zero dealable cards still returns `NO_CARDS_AVAILABLE`.
- `GET /api/sets/:id` (public) — set metadata, maker + co-creator usernames, `createdAt` (drizzle Date → ISO), eligible `cardCount` (same deal predicate as `GET /api/sets`, not a raw `is_playable` count), `shareImageUrl` when a runtime maker-share PNG exists (looked up for any set, not only `isUserCreated`; stock fan stripped), `previewCards: { imageUrl, year }[]` using the same valid pinned cover URLs (set year only; no player, description, raw image URL, or card id), and `playedToday` for the session user (CT day; guests are `false`). Public authored date renders only for a user-created set with a maker username (`formatAuthoredDate` → `formatPackptsMonDay`, America/Chicago). Integrated sets omit `by Maker`, the date, and AUTHORED. `cardCount` uses `eligiblePlayableCardCountSql`; `playCount` stays the correlated `COUNT(*)::int` against `game_sessions` using the raw identifier `game_sets.id` (`server/routes/userSetCounts.ts`). Interpolating `${gameSets.id}` inside drizzle `sql` templates rebinds the column as a parameter and reports `0` even when the set has playable cards. No denormalized `sets.cardCount` column. Cover helpers: `server/routes/userSetPreview.ts`, `server/services/setCovers.ts`. List and detail handlers: `server/services/publicSets.ts`. `GET /api/sets/:setId/covers/:slot` streams the baked JPEG and does not bake. A successful cover response sends `Cache-Control: public, max-age=86400, stale-while-revalidate=604800`. A missing sidecar is `404` with `Cache-Control: private, no-store`. `SETS_COVERS_DISABLED=1` or `true` returns empty cover arrays from the list and detail (and omits `shareImageUrl`, with `coversDisabled: true`) and 404s every cover request with `Cache-Control: private, no-store, no-cache, must-revalidate` plus `CDN-Cache-Control: no-store` and `Surrogate-Control: no-store`. Any other value leaves this path unchanged. A successful cover response also sends `X-Card-Id` (the baked card) and `X-Mask-Version`. A 404 removes `X-Card-Id`.
- `GET /api/qa/sets/:setId/cover-candidates` (header `X-QA-Token`) — Design review of the same cover picker. Off unless `COVER_QA_TOKEN` is non-empty. A missing or wrong header, or a token only in the query string, is 404 `{ error: "Not found" }`. `n` defaults to 12 and caps at 24. JSON is `{ setId, maskVersion, pinnedCount, validCount, coversDisabled, pins, picker, candidates }`. `candidates` are the valid pins in order (`source: "pinned"`, `served: true`). `picker` is the old auto order (`source: "picker"`) so Design can see what was not pinned. `pins` lists each configured id with `status` `served` or `dropped`, a `reason` when dropped, `slot`, `served`, and `inPicker`. `coversDisabled` reports the master switch and does not empty this payload. Each candidate still has `slot`, `cardId`, `gameSetId`, `player`, `number`, `variant`, `maskVersion`, `bandPlacement`, `baked`, and `imagePath`. Public slots are the valid pins compacted from 0. A failed pin is not replaced. `bandPlacement` is `TOP_PLATE`, `BOTTOM_PLAQUE`, `PSA_SLAB`, or null. `baked` is true. `imagePath` is `/api/qa/cover-image/{cardId}`. Responses send `Cache-Control: private, no-store, no-cache, must-revalidate`, `CDN-Cache-Control: no-store`, `Surrogate-Control: no-store`, and `X-Robots-Tag: noindex`. This route ignores `SETS_COVERS_DISABLED`.
- `GET /api/qa/cover-image/:cardId` (header `X-QA-Token`) — the same baked masked JPEG `resolveReadyWarmMaskedFile` serves on the public cover route for that card, with `Content-Type: image/jpeg`, `X-Card-Id`, and `X-Mask-Version`, plus the same no-store and `X-Robots-Tag: noindex` headers. A card that fails the cover filters (blocklist, band guard, deal eligibility, missing sidecar) is 404 and is never the raw scan. This route ignores `SETS_COVERS_DISABLED`.
- `GET /api/qa/sets` (header `X-QA-Token`) — active integrated sets and their dealable totals. Off unless `COVER_QA_TOKEN` is non-empty. A missing or wrong header, or a token only in the query string, is 404 `{ error: "Not found" }`. JSON is `{ maskVersion, sets: [{ setId, setName, total }] }`, ordered by set name then set id. `total` is `count` of `eligibleDealFilter("playable_cards")` for that set (blocklist, subset quarantine, mask `blocked_reason`, and current-mask fail sidecars included). Inactive and user-created sets are omitted. Same no-store and `X-Robots-Tag: noindex` headers as the other QA routes. Ignores `SETS_COVERS_DISABLED`.
- `GET /api/qa/sets/:setId/dealable-cards` (header `X-QA-Token`) — every card in that set's deal pool, not the 24-card cover cap. Same token gate and 404. `offset` defaults to 0. `limit` defaults to 200 and caps at 500. A set that is missing, inactive, or user-created is 404. JSON is `{ setId, setName, maskVersion, total, offset, limit, cards: [{ cardId, player, number, variant, bandPlacement, baked }] }`. The WHERE clause is `eligibleDealFilter("playable_cards")` on that `game_set_id`. A playable row with a `{cardId}_v4.6.fail` sidecar is excluded by that filter. Order is `number`, then `cardId`. `bandPlacement` is `TOP_PLATE`, `BOTTOM_PLAQUE`, `PSA_SLAB`, or null. `baked` is true when the current-version masked sidecar is ready. Same no-store headers. Ignores `SETS_COVERS_DISABLED`.
- `GET /api/qa/cover-image/:cardId` (header `X-QA-Token`) — the v4.6 masked JPEG for any card that passes `eligibleDealFilter`, with `Content-Type: image/jpeg`, `X-Card-Id`, and `X-Mask-Version`, plus the same no-store and `X-Robots-Tag: noindex` headers. A ready sidecar is streamed. If it is not baked yet, the route calls `getMaskedImagePath` (live bake queue, timeout, concurrency cap) and streams only a `{cardId}_v4.6.jpg` (or `_r90` / `_r180` / `_r270`) file. A bake timeout is 503 `{ error: "Masked image temporarily unavailable" }` with `Retry-After: 5`. A card that fails the deal filter, including a current-mask fail sidecar, is 404. The raw scan is never sent. This route ignores `SETS_COVERS_DISABLED`. Registered ahead of the pinned-cover image handler. A card that already passes those cover filters is streamed from the baked file with no bake. A dealable card with no sidecar is baked here. The non-baking handler stays registered. Public `/api/sets/:setId/covers/:slot` does not bake. `SETS_COVERS_DISABLED` still hides only the public cover route.
- `POST /api/qa/rejected-cards/backfill` (header `X-QA-Token`) — dry-run backfill of `mask_bake_refusals` for active integrated cards that already failed the current mask (`{cardId}_v4.6.fail` or a mask `blocked_reason`) and do not yet have a v4.6 row. Same token gate and 404. A query-string token is ignored. `setId` limits the set. `limit` defaults to 50 and caps at 200. JSON is `{ scanned, inserted, skipped, errors, remaining }`. Pinned cover ids are processed first. A plate that now resolves is inserted with reason `resolves_now` and the card stays refused. No masked JPEG, sidecar, `card_image_mask_cache` row, or `blocked_reason` change. Uses the warm bake slot and the player bake deadline. Same no-store headers. Ignores `SETS_COVERS_DISABLED`.
- `GET /api/qa/rejected-cards` (header `X-QA-Token`) — rows from `mask_bake_refusals`. Off unless `COVER_QA_TOKEN` is non-empty. A missing or wrong header, or a token only in the query string, is 404 `{ error: "Not found" }`. `setId` filters `game_set_id`. `offset` defaults to 0. `limit` defaults to 200 and caps at 500. JSON is `{ maskVersion, total, offset, limit, refusals: [{ id, cardId, setId, reason, layoutClass, profileSource, expectedPlate, ocrBoxes, candidates, maskVersion, createdAt, imageWidth, imageHeight, debugPath, sourcePath, attemptPath }] }`. The list does not include the source bytes. Same no-store headers and `X-Robots-Tag: noindex`. Ignores `SETS_COVERS_DISABLED`.
- `GET /api/qa/rejected-cards/:cardId/debug.png` (header `X-QA-Token`) — the stored upright source with the expected plate (gold), candidate boxes (cyan), and OCR boxes (magenta) drawn and labeled. `Content-Type: image/png`. Same token 404 and no-store headers, plus `Content-Security-Policy: default-src 'none'` and `X-Content-Type-Options: nosniff`. No `X-Mask-Cache`. Not written to the masked-card directory.
- `GET /api/qa/rejected-cards/:cardId/source` (header `X-QA-Token`) — the stored upright source bytes, `Content-Type` from the row. Same token 404 and no-store headers. Not a player route and not the masked-image cache.
- `GET /api/qa/rejected-cards/:cardId/attempt.png` (header `X-QA-Token`) — a dry-run of the regions stored with the refusal (what v4.6 would have painted if that plate had been accepted). `Content-Type: image/png`. Same token 404 and no-store headers. Generated in memory. Not written to the masked-card directory.
...
- `/sets` — play shelf for integrated sets already in PackPTS (`docs/SETS_POLISH.md`): eyebrow SETS, H1 “Sets”, sub “Play sets already in PackPTS.”, honest `{n} sets` from that list, Surface A cover crop or a fanned stack of baked masked cover URLs whose plaque is a solid bar with the gold seam and no label text. When `SETS_COVERS_DISABLED` is `1` or `true`, or the set has no valid covers, the tile does not render a cover slot. It shows the set name, the year label, and Play this set. No card count, image, gray box, cream card, or placeholder line. One to eight valid covers fan at the normal cover height, spread evenly, with no empty slot. Integrated meta is `{n} cards` only (no `by Maker`, no date, no AUTHORED), blue **Play this set**, no short-shelf banner, empty state only at zero sets, Play Daily 5 (no Make a set). A stored brand missing from the set name is inserted into the heading (`2024 Basketball` + Topps → `2024 Topps Basketball`). Set `229f0379-aa56-40a8-abe3-1af217a397e8` displays as `2024-25 Topps Chrome Basketball` with year label `2024-25` via `shared/setDisplayOverride.ts`. The stored row is unchanged. No search chrome, no play counts, no Maker Rate, no playable-card vanity total. Also in mobile nav ("Browse") and home game-modes grid.
- `/sets/:id` — public set page. With valid pinned covers: provenance, mixtape, honest card count + Play, optional “Play today’s stack” if not played today (CT), Surface A / runtime cover, THE STACK (masked, no player names), Share / Copy link / `packpts.com/sets/{slug}`. With no valid covers, or when `SETS_COVERS_DISABLED` is on: set name, year label, and Play only. No placeholder cards, gray boxes, card count, or coming soon. Share and copy use `playSetsShareUrl` (`utm_source=share&utm_medium=play_sets&utm_campaign=integrated`). HTML OG on `/sets` and `/sets/{slug}` prefers that runtime cover. No Times Played tiles, currency chrome, or “Make another set” / `/make` CTA. Maker / co-creator get a quiet Save cover link for the Surface A PNG.
