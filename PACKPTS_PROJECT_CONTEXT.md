...
- **Post-submit full-card reveal (2026-09-16):** Dave’s rule: mask stays until a **successful** answer submit (correct or wrong), then the **full** card is shown; the next card starts masked. Guessing phase keeps the current bake (`/api/cards/:id/masked-image?v=v4.2`) plus the GameCard overlay. After ACK, parents swap `img src` via `resolvePlayCardSrc` to `/api/images/card/:id` and drop the overlay (`isRevealed`). Solo sets `isRevealed` in submit `onSuccess` only (not on Submit click). Daily 5 `onSuccess`. 1v1 `answerResult !== null`. 1v1 replace uses `maskedCardImageUrl`. WS `sanitizeMatchStateForClient` sends `gameSetId` so overlay profiles match the set. `GameCard` stays dumb (renders the URL it is given). Do not weaken localization to fake a reveal. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`.
...
| GET | /api/daily5/status | No* | Today’s challenge + existing entry (`answers[]`, score). Client resumes from this. |
...
### Gameplay
- [x] Brand mark dual-logo (2026-09-20): header shipped the glossy 3-card shield (`packpts-logo.png`) into app thumbnails. Two-role SoR: **B masked-P** on header/favicon/PWA/manifest (SVG + existing rasters, byte-match live); **A masked-card** stays on OG/social only. Shield deleted. Manifest now lists `icon-512-maskable.png`. X avatar hosted at `/assets/brand/playpackpts-avatar-masked-p-1024.png`. `packpts-design/` kill-list was not on disk; no new logo invented.
