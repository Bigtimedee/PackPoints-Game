...
**Last verified against codebase:** 2026-09-08 (make empty/retry + MAKE_FRICTION funnel)
...
- [x] `/make` empty-state + identify-retry (2026-09-08): no-draft chrome is Design EMPTY_STATE (`docs/design/EMPTY_STATE.md`) — SNAP-TO-SET / Photo the stack / example cream-masked stack / EXAMPLE · NOT YOUR PC / Take photo + Choose from library. Soft auth: `Sign in to photo your stack.` MAKE_FLOW auth-before-upload unchanged. Failed identify slots: `Couldn't identify` + **Try again** + Skip, quiet gold-40% border, no alarm red or toast-per-failure; sequential queue not blocked (Design IDENTIFY_RETRY: `docs/design/IDENTIFY_RETRY.md`). Empty chrome is not Surface A share art.
...
- `/make` — 3-step wizard (Upload → Review → Publish) per MAKE_FLOW: auth-before-upload, two file-input CTAs, sequential identify, then name/mixtape → publish → Surface A share. No-draft step 1 uses Design EMPTY_STATE (example PC stack + Take photo / Choose from library; signed-out `Sign in to photo your stack.`). Draft rows use Design IDENTIFY_RETRY (Queued / Identifying… / green-check success / `Couldn't identify` + Try again + Skip). "Make it together" creates a collab session. After publish, default share preview is the runtime **I MADE THIS SET** PNG (that set’s masked cards + name + mixtape), not stock fan art. Contracts: `docs/design/EMPTY_STATE.md`, `docs/design/IDENTIFY_RETRY.md`. Funnel: `event_log` via `/api/make/event`.
- `/sets` — community browse grid with search; also in mobile nav ("Browse") and home game-modes grid.
- `/sets/:id` — public set page; shows "by {maker} & {co-creator}". Maker / co-creator also get the same share PNG to re-download.
