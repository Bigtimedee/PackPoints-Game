...
**v4.5 (2026-09-25):** `CURRENT_MASK_VERSION` is `v4.5`. Each successful bake writes `layoutClass` + `regions` onto `card_image_mask_cache` (nullable columns) and a sidecar `data/masked-cards/{cardId}_v4.5.json`. Play payloads attach `maskPlan` from that sidecar or `null` when the bake is cold. The wire plan never includes player name, card number, or team. Year+brand profiles (`1987 topps` and the other baseball named keys) apply only when sport is confirmed baseball. `1987 Topps Football` stays `TOP_PLATE` by `gameSetId`. Unmatched profile + no OCR name hit returns `UNKNOWN`, fails the coverage gate, and `shouldDealMaskedCard` skips those cards in solo, Daily 5, and 1v1. Daily 5 still has no replacement copy. Reveal stays the #116 token URL, mounted only after ACK, and a reveal `onError` does not call `onImageError`. Solo `+N pts` renders under the answer row from server `pointsEarned`, with no bounce and no overlay on the card.
