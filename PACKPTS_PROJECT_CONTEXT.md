...
**Last verified against codebase:** 2026-09-08 (staff QA identify-fail on `/make`)
...
- [x] `/sets` Goldin-quiet polish (2026-09-08): index + detail match `docs/SETS_POLISH.md` (Maker sets, Surface A cover else masked stack, honest `{n} cards`, short-shelf banner below the ≥10 gate, no Times Played / Maker Rate / currency chrome). Play uses the set’s real card count (5–20), not a hardcoded 10, and resumes that session on `/game/solo?session=` so the setup picker is skipped.
...
- `/sets` — Goldin-quiet Maker sets shelf (`docs/SETS_POLISH.md`): eyebrow SETS, H1 “Maker sets”, sub “Built from the PC. Authored, not scrolled.”, Surface A cover crop or masked stack, `by {maker} · {n} cards · {date} · AUTHORED`, blue Play, short-shelf banner when published count is below 10, Make a set + Play Daily 5. No search chrome, no play counts, no Maker Rate. Also in mobile nav ("Browse") and home game-modes grid.
