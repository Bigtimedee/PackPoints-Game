...
- [x] Maker share v2 (2026-09-08): Surface A — after `/make` publish succeeds, default share art is a runtime 1080 PNG stack of that set’s masked cards (Daily 5 `WHO IS THIS PLAYER?` language) + set name + mixtape + `packpts.com/sets/{setSlug}`. Mask failure → cream silhouette per card; never `maker-set-1080.png`. Sets Made optional after ≥10 non-staff sets; no Maker Rate. Contract: `docs/MAKER_SHARE_CONTRACT.md`. Collab templates stay.
...
After a user publishes via Snap-to-Set (`POST /api/sets/create` only), `onSetPublished` writes a 1080×1080 PNG (`content_assets.asset_type = MAKER_SHARE_CARD`, `source_event_id = maker_set_{setId}`). Layout: `I MADE THIS SET`, published set name + mixtape, a stack of 3–8 of **this set’s** identified cards as Daily 5 masked cards (`WHO IS THIS PLAYER?` name band). Prefer `card_photos` thumbnails; if a card’s mask/photo fails, that slot is a cream silhouette (`#F3E6C8`) — never the stock fan `maker-set-1080.png`. Footer: masked-P + PackPTS + `packpts.com/sets/{setSlug}`. Optional personal `N sets made` only after ≥10 non-staff published user sets; Maker Rate / volume brags stay gated. Collab / badge templates are out of scope. Locked spec: `docs/MAKER_SHARE_CONTRACT.md`.
