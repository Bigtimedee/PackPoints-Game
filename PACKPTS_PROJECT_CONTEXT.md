...
- **Set rotation:** One set per CT day, same five cards for everyone that day. The roster is `loadActiveIntegratedSets` (active, not user-created, deduped by name + year + sport, eligible count from `eligiblePlayableCardCountSql`). Order is `created_at` ascending, then set id. Index is calendar days since `2020-01-01` (`DAILY5_ROTATION_EPOCH`), mod the roster length. A set with fewer than 5 eligible cards is skipped and the next set is used. Cards in the set go through `eligibleDealFilter` (the shared solo / `/api/sets` predicate, including `is_playable`) plus that set's sport, then the silhouette and non-player pass. There is no post-bake name-check column yet; when it is added to `eligibleDealFilter`, Daily 5 picks it up. A `daily_challenges` row that already exists for that date is returned as stored, including `daily_challenge_cards`. Adding or removing a set does not rewrite that row. `GET /api/daily5/status` includes `setName` on the body and on `challenge` only when `game_sets.title_verified` is true for that set. Otherwise `setName` is null. It does not add player names, card ids, card counts, or points. Beat-me `puzzle_day`, the Daily 5 share card, and 1v1 stay on this same CT day. The share card still says Daily 5 and prints the scored total. When the title is verified, the set name is the line under that score, and the share text is `Daily 5. {title}. {n}/5.` 1v1 keeps the `1V1 MATCH` chrome and the same scored total for that count.
- **Set label:** In play and on reveal, one small-caps muted line sits under the Daily 5 title: `TODAY'S SET` then two spaces then the set title in caps. Game Complete uses that same line above the score. A null `setName` renders no line and no placeholder. The line is not on the card. It stays out of the answer, Next Card, and Report Wrong Image stack. It wraps inside 320px and is not truncated. `title_verified` defaults false. Boot runs `backfillVerifiedSetTitlesOnce` once, while no row is verified, and sets the flag true for active sets except set name `2024 Basketball` and ids starting `229f0379`. Later sets stay false until an admin sets the flag.
...
- `game_sets` gained: `created_by_user_id` (FK users, null for staff sets), `co_creator_user_id` (FK users, set by collab publish), `maker_note` (text, ≤140 chars enforced at API layer), `is_user_created` (boolean, indexed). `title_verified` (`titleVerified`, boolean, default false) gates the public Daily 5 set label. See Daily 5 set label.
...
- `/sets` — play shelf for integrated sets already in PackPTS (`docs/SETS_POLISH.md`): eyebrow SETS, H1 “Sets”, sub “Play sets already in PackPTS.”, honest `{n} sets` from that list, Surface A cover crop or a stack of baked masked cover URLs (cream placeholder when no sidecar is ready; the plaque is decoration), integrated meta is `{n} cards` only (no `by Maker`, no date, no AUTHORED), blue **Play this set**, short-shelf banner when the list is below 10, empty state only at zero sets, Play Daily 5 (no Make a set). No search chrome, no play counts, no Maker Rate. Also in mobile nav ("Browse") and home game-modes grid.
