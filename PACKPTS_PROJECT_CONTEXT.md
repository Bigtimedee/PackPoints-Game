...
- **Stale bundle:** `client/src/lib/staleBuildClient.ts` compares the inlined build id with `GET /api/version?t=<ms>` (`cache: 'no-store'`). The poll is a 60s interval plus focus, visibility, pageshow, and online. A live question defers `location.reload()` until Next Question, Game Complete, or a real navigation (`shared/buildVersion.ts`). One reload per build id per tab. Chunk load failures use that same safe reload.
...
3. Confirm `buildId` is present, `Cache-Control` contains `no-store`, and there is no `ETag`. A tab opened before this deploy polls every 60s and on focus/visibility/online. It reloads on navigation, Next Question, or Game Complete. It does not reload mid-question or during an answer submit, and not twice for the same id.
