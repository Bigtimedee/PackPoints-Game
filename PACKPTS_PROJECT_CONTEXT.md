...
**Last verified against codebase:** 2026-09-16 (PSA-slab cert-label bake v4.2; Social Media Agent Marketing SoR Daily 5-only auto X + FOMO preflight #91; play-sets CDN aliases #90; post-submit full-card reveal. Play Again #87; name masking #86; Daily 5 overlay #85)
...
- **Server JPEG:** `maskingService.ts` + `maskCardImage.ts` bake opaque name-region overlays into `/api/cards/:id/masked-image?v=v4.2`. Cache: `card_image_mask_cache.maskVersion` + filename `{cardId}_v4.2.jpg` on the Railway volume. HTTP `ETag` / `X-Mask-Version` follow the bake id. The route ignores `?v=` for file lookup — gameplay URLs must bump `CURRENT_MASK_VERSION` so CDNs fetch a new object. Rebuild: `POST /api/admin/masks/rebuild` `{ "all": true }` (or `{ "cardIds": ["…"] }`) — `docs/MASK_CACHE_REBUILD.md`. Overlay stays set-level; per-card slab labels are covered by the baked JPEG.
