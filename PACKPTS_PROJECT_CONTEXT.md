...
- **Intelligent name localization (v4.1, 2026-09-16):** Masking finds where the **player name** lives on that card, then covers that region. It is not a universal bottom wipe. Chain: PSA/slab layout (red header + white cert plate, or OCR `PSA`/`BGS`/`SGC` in the top third) → OCR name/jersey tokens (Tesseract, last-name + fuzzy match) → known layout profile (`server/masking/maskProfiles.ts`, e.g. 1986–1990 Fleer Basketball top name plate) → `DEFAULT_MASK_REGIONS` bottom plaque for unknown sets. v4.0 fixed Dave’s 1989 Fleer Spud Webb leak (top name plate). v4.1 covers **grader-slab certificate labels** (Design QA: 1987 Topps Tiffany PSA 9 Roger Clemens still showed **ROGER CLEMENS** on the top PSA label while the bottom ~60% was masked). Slab geometry: full-width top ~22% cert band **plus** the set plaque (1987 Topps bottom 46% stays). Raw Fleer top-name is unchanged. Helper: `server/masking/slabLayout.ts`.
...
- **Server JPEG:** `maskingService.ts` + `maskCardImage.ts` bake opaque name-region overlays into `/api/cards/:id/masked-image?v=v4.1`. Cache: `card_image_mask_cache.maskVersion` + filename `{cardId}_v4.1.jpg` on the Railway volume. Rebuild: `POST /api/admin/masks/rebuild` `{ "all": true }` — `docs/MASK_CACHE_REBUILD.md`. Overlay stays set-level; per-card slab labels are covered by the baked JPEG.
...
- **Post-submit full-card reveal (2026-09-16):** Dave’s rule: mask stays until a **successful** answer submit (correct or wrong), then the **full** card is shown; the next card starts masked. Guessing phase keeps the current bake (`/api/cards/:id/masked-image?v=v4.1`) plus the GameCard overlay. After ACK, parents swap `img src` via `resolvePlayCardSrc` to `/api/images/card/:id` and drop the overlay (`isRevealed`). Solo sets `isRevealed` in submit `onSuccess` only (not on Submit click). Daily 5 `onSuccess`. 1v1 `answerResult !== null`. 1v1 replace uses `maskedCardImageUrl`. WS `sanitizeMatchStateForClient` sends `gameSetId` so overlay profiles match the set. `GameCard` stays dumb (renders the URL it is given). Do not weaken localization to fake a reveal. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`.
...
- [x] Play-sets CDN short-name trap (Design QA 2026-09-16): live `integrated-shelf.png` and `*-story.png` were real PNGs; `integrated-set.png`, `integrated-beatme.png`, `play-shelf.png`, `play-set-1080.png`, `play-shelf-1080.png`, `play-beatme-1080.png`, `set-1080.png`, `beatme-1080.png` returned SPA HTML. Same class as the 2026-09-13 x-hotfix: Design/marketing requested names Eng did not host. Fix: copy the canonical kit bytes to those exact aliases (`PLAY_SETS_CDN_ALIASES`). Do not invent extra short names.
...
- [x] PSA-slab top-label leak (Design QA 2026-09-16): v4.0 bake masked the inner 1987 Topps plaque but left **ROGER CLEMENS** on the PSA cert label. v4.1 detects slab layout (PSA red header + white plate, or OCR grader token) and unions a top ~22% cert band with the set plaque. Fleer top-name raw cards stay top-only. Rebuild: `POST /api/admin/masks/rebuild` `{ "all": true }` — `docs/MASK_CACHE_REBUILD.md`.
...
Marketing + in-product share for sets already on `/sets`. Surfaces: **A Play this set** · **B Integrated shelf** · **C Beat me from a set**. `beat_me` / `beat-me` / `beat_me_from_a_set` → C. CTA lock: `packpts.com/sets` or `packpts.com/sets/{slug}` only, with `utm_source=share&utm_medium=play_sets&utm_campaign=integrated`. `set` or `slug` resolves UUID or public slug (URLs cleaned) to the integrated set and prefers runtime cover. Kit PNGs at `/assets/play-sets/play-this-set.png`, `/assets/play-sets/integrated-shelf.png`, `/assets/play-sets/beat-me-from-a-set.png` plus Design story crops `/assets/play-sets/play-set-story.png`, `/assets/play-sets/play-shelf-story.png`, `/assets/play-sets/play-beatme-story.png` (`format=story`). CDN aliases (same bytes): `integrated-set.png`, `integrated-beatme.png`, `play-shelf.png`, `play-set-1080.png`, `play-shelf-1080.png`, `play-beatme-1080.png`, `set-1080.png`, `beatme-1080.png`. No `/make` publish CTA, no Maker Rate / public volume claims. Surface C does not mint a Daily 5-style challenge token. Contract: `docs/PLAY_SETS_SHARE.md`.
