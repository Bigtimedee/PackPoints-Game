...
- **Surname outside the plate (2026-09-26, check `n1`, files stay v4.5):** `verifyNameVisibleOutsideMask` OCRs the whole masked JPEG and fuzzy-matches the player surname and full name. Short surnames (under 5 letters) and common surnames match only as a whole word. Longer surnames also match a Levenshtein ratio of 0.8 and a 5-letter run, so a split or noisy `ANTETOKOUNMPO` on a jersey still fails. A hit outside the mask regions writes `{cardId}_v4.5.fail` with `name_visible_outside_mask`, sets `blocked_reason` to that value, drops `is_playable`, and removes the `.ok` sidecar. The bake does not paint over the jersey. The OCR does not run inside the masked-image request. Warm-up walks existing v4.5 JPEGs (today's Daily 5 cards first) and a finished bake schedules the same check after the response is free to return. Until that check finishes, a warm file can still be served once. After the fail sidecar exists, `peekWarmMaskedFilename` returns null and a rebake is refused, so the route is 422 `mask_name_uncovered` with the reason `name_visible_outside_mask` and does not hang. `CURRENT_MASK_VERSION` stays `v4.5`.
