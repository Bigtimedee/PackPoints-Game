...
**Last verified against codebase:** 2026-09-11 (play-integrated set share kit: runtime cover OG + locked play_sets UTMs)
...
| GET | /api/share/play-sets | No | Marketing play-sets share JSON: destination (`/sets` or `/sets/{slug}` + locked play_sets UTMs), `imageKind` `runtime`\|`kit`, kit URL, OG fields. Query: `surface`, `set`, `asset` |
| GET | /api/share/play-sets/image | No | PNG — kit template or runtime cover/crop for a known set |
...
Marketing + in-product share for sets already on `/sets`. Surfaces: **A Play this set** · **B Integrated shelf** · **C Beat me from a set**. CTA lock: `packpts.com/sets` or `packpts.com/sets/{slug}` only, with `utm_source=share&utm_medium=play_sets&utm_campaign=integrated`. Runtime cover / masked-stack crop wins for per-set share, OG, and `/sets` heroes; kit PNGs at `/assets/play-sets/play-this-set.png`, `/assets/play-sets/integrated-shelf.png`, `/assets/play-sets/beat-me-from-a-set.png` are cold posts / placeholders until that set’s runtime art exists. Marketing: `GET /api/share/play-sets?surface=&set=&asset=kit|runtime`. No `/make` publish CTA, no Maker Rate / public volume claims. Surface C does not mint a Daily 5-style challenge token. Contract: `docs/PLAY_SETS_SHARE.md`.
