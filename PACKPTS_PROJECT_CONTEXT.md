...
- **Server JPEG:** `maskingService.ts` + `maskCardImage.ts` bake solid RGB name-region overlays (`#0a0e16`, no alpha) into `/api/cards/:id/masked-image?v=v4.4`. Cache: `card_image_mask_cache.maskVersion` + filename `{cardId}_v4.4.jpg` on the Railway volume. Warm hits peek that file and skip DB/OCR (`X-Mask-Cache: hit`). HTTP `ETag` / `X-Mask-Version` follow the bake id; `Cache-Control` is `public, max-age=86400, stale-while-revalidate=604800`. The route ignores `?v=` for file lookup — gameplay URLs must bump `CURRENT_MASK_VERSION` so CDNs fetch a new object. A bake that fails `assertOpaqueIdentityCover` is not written; the route returns 422 `mask_name_uncovered` and the card is quarantined. Rebuild: `POST /api/admin/masks/rebuild` `{ "setId": "…" }` — `docs/MASK_CACHE_REBUILD.md`. Overlay stays set-level; per-card slab labels are covered by the baked JPEG.
- **Between-card lag (2026-09-19):** Guessing used to fetch the next masked JPEG only when that card mounted. Daily 5 and 1v1 had no prefetch; solo prefetched only N+1 via `card.imageUrl`. `preMaskCards()` was unused, so the first GET of each card could cold-bake (download + OCR). Now: client `prefetchMaskedPlayCards` loads remaining **masked** URLs as soon as the deal is known (1v1 gets `upcomingMaskedCardIds` — ids only, no names). Reveal `/api/images/card/:id` is prefetched only after a successful submit. Server `kickPreMask` fires on solo start/replace, Daily 5 create/start, and 1v1 start/replace — background, not on the start-response path. Solo Next paints the next known card from session state immediately, then reconciles `/api/game/next` (submit stays blocked until ACK). GameCard skips the spinner when that URL is already decoded. Volume-cold after mask rebuild or a new Daily 5 day: first request still bakes; document in `docs/MASK_CACHE_REBUILD.md`.
...
- **Post-submit full-card reveal (2026-09-16):** Dave’s rule: mask stays until a **successful** answer submit (correct or wrong), then the **full** card is shown; the next card starts masked. Guessing phase keeps the current bake (`/api/cards/:id/masked-image?v=v4.4`) plus the GameCard overlay. After ACK, parents swap `img src` via `resolvePlayCardSrc` to `/api/images/card/:id` and drop the overlay (`isRevealed`). Solo sets `isRevealed` in submit `onSuccess` only (not on Submit click). Daily 5 `onSuccess`. 1v1 `answerResult !== null`. 1v1 replace uses `maskedCardImageUrl`. WS `sanitizeMatchStateForClient` sends `gameSetId` so overlay profiles match the set. `GameCard` stays dumb (renders the URL it is given). Do not weaken localization to fake a reveal. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`. **2026-09-24:** The reveal stays in that same in-flow slot (`data-testid="solo-card-slot"`). There is no reveal dialog and no click-to-enlarge. Revealed answer buttons stay `disabled` but use `disabled:opacity-100` so the correct/incorrect row is not painted at half opacity (that read as a scrim). Do not wrap the card in a modal on submit.
- **Solo / 1v1 per-card “pts” badge removed (2026-09-24):** Solo rendered `Worth {question.pointValue} pts`. Playable-set deals set `pointValue` from `computeReward({ playerName, sport: "baseball" })` with no year and no rarity (`generateQuestionFromPlayableCard` stores `year: 0`). A missing fame row is 0.5, and the default policy maps that to exactly 175, so every such card showed the same number. Authenticated credit is `awardDailyBaseForCorrectCard` (`deltaPts` after Set of the Week and the daily cap), not that badge. A flat default is not a per-card price, so the badge is gone. The post-ACK `+points` animation still shows server `pointsEarned`. Daily 5 never rendered a per-card worth label (it credits a flat `pointValue: 100`). 1v1 credits `question.pointValue`, but playable cards are stamped `popularity: 50`, so the `{pointValue} pts` badge was a constant 300; that badge is removed too. Do not put a client-invented number back.
...
**v4.4 geometry:** The band is **not** hardcoded to `bottom: 0, height: 46%`. It follows overlay regions from mask config (top plate for 1989 Fleer Basketball and 1987 Topps Football, bottom plaque for 1987 Topps baseball, OCR boxes when present). Lookup is `gameSetId` then sport+year+brand. Year+brand alone must not paint football with the baseball plaque. PSA-slab bakes add a top cert-label region on the JPEG; the set overlay can stay plaque-only. If config is empty, `overlayMaskRegions()` falls back to `DEFAULT_MASK_REGIONS`. Never remove the always-on render; never re-hardcode a bottom-only band.
...
- **Stale bundle:** `client/src/lib/staleBuildClient.ts` compares the inlined build id with `GET /api/version`. Reload is deferred off live play (`shared/buildVersion.ts`). Chunk load failures reload once.
...
- **Health:** `/api/health`, `/api/version` (`buildId` matches the served client; `Cache-Control: no-store`)
...
The database has **144+ tables** defined in `shared/schema.ts` using Drizzle ORM. `start.sh` runs `drizzle-kit push --force`, which drops tables that are not in that file. `job_queue`, `promotions`, `user_attribution`, `creator_applications`, `partner_inquiries`, and `user_feedback` are declared there for that reason. `refresh_tokens`, `apns_tokens`, and `apple_transactions` were already declared. Below are the major domain groups with key tables.
...
`GET /api/version` is the deploy verification canary. The response includes the short git SHA (`sha`) and the client build id (`buildId`). `Cache-Control` is `no-store`.
...
3. Confirm `buildId` is present and `Cache-Control: no-store`. A tab opened before this deploy reloads onto it at the next safe moment (navigation, or focus outside a live game). It does not reload mid-card, during an answer submit, or twice for the same id.
...
- [x] Post-submit full-card reveal (Dave, 2026-09-16): after a successful answer submit the overlay drops **and** `img src` swaps to `/api/images/card/:id`; guessing stays the current bake + overlay. Solo `setIsRevealed(true)` is ACK-only. 1v1 replace uses `maskedCardImageUrl`. WS passes `gameSetId`. Helper `shared/playCardImage.ts`. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`. Do not regress #85 / #86 / #87. **2026-09-24:** Reveal stays in the in-flow card slot. No dialog, no zoom. Revealed choices use `disabled:opacity-100`.
- [x] Static “Worth 175 pts” (Dave, 2026-09-24): solo badge removed. Playable-set `pointValue` is `computeReward` with no year/rarity; default fame 0.5 → 175 on the default policy, and auth credit is `awardDailyBaseForCorrectCard` (set bonus + daily cap), not the badge. 1v1’s `{pointValue} pts` badge removed for the same reason (`popularity: 50` → constant 300). Daily 5 has no per-card worth label. Post-ACK `+points` still shows server `pointsEarned`.
...
- [x] Between-card lag (Dave, 2026-09-19): next masked JPEG waited for submit ACK + first GET (often a cold bake). Client now prefetches remaining masked URLs when the deal is known; 1v1 sends `upcomingMaskedCardIds`. Server `kickPreMask` on solo/Daily 5/1v1 start. Warm `/masked-image` is a disk peek (`X-Mask-Cache`). Solo Next shows the next card shell immediately. Reveal URL stays post-submit only. Do not regress #85 / #86 / #87 / v4.2 geometry / social SoR.
...
- [x] FIFO bucket expiration job is scheduled via pgJobQueue (`packpts_expiration`) and runs daily at `EXPIRATION_RUN_HOUR_UTC` (default 6 UTC). The scheduled run defaults to `EXPIRATION_MODE=dry_run`. Inactivity expiration still runs only via manual trigger / standalone script.
...
- `GET /api/sets` (public) — browse user-created sets ordered by `created_at` DESC (authored shelf, not a play-count board); powers `/sets`. Each row includes honest `cardCount`, `shareImageUrl` (Surface A `content_assets` lookup; stock fan `maker-set-1080.png` stripped), `coverCardUrls` (up to 8 playable image URLs, no player names), and `createdAt` as ISO UTC (`createdAtToIso` — raw `db.execute` otherwise returns a naive `YYYY-MM-DD HH:MM:SS` that browsers parse as local). `playCount` remains on the JSON for existing callers; public `/sets` UI must not render it.
- `GET /api/sets/:id` (public) — set metadata, maker + co-creator usernames, `createdAt` (drizzle Date → ISO), honest `cardCount`, `shareImageUrl` when a runtime maker-share PNG exists (looked up for any set, not only `isUserCreated`; stock fan stripped), `previewCards: { imageUrl, year }[]` (no player / description), and `playedToday` for the session user (CT day; guests are `false`). Public authored date on `/sets` and `/sets/:id` is `formatAuthoredDate` → `formatPackptsMonDay` (America/Chicago). `cardCount` / `playCount` are correlated `COUNT(*)::int` subqueries against `playable_cards` / `game_sessions` using the raw identifier `game_sets.id` (`server/routes/userSetCounts.ts`). Interpolating `${gameSets.id}` inside drizzle `sql` templates rebinds the column as a parameter and reports `0` even when the set has playable cards (live miss on Design QA Sep8 Stack `17e5d554-…` — share PNG and `GET /api/sets` browse were already honest; the public set page was not). No denormalized `sets.cardCount` column; no backfill — existing rows self-heal on the next GET. Cover helpers: `server/routes/userSetPreview.ts`.
...
- `/sets` — play shelf for sets already in PackPTS (`docs/SETS_POLISH.md`): eyebrow SETS, H1 “Sets”, sub “Play sets already in PackPTS.”, Surface A cover crop or masked stack, `by {maker} · {n} cards · {MON D CT} · AUTHORED`, blue Play, short-shelf banner when published count is below 10, Play Daily 5 (no Make a set). No search chrome, no play counts, no Maker Rate. Also in mobile nav ("Browse") and home game-modes grid.
