...
**Last verified against codebase:** 2026-09-08 (maker-supply funnel + admin retention)
...
- [x] Maker-supply instrument (2026-09-08): `event_log` funnel `/make` start → identify success/fail → publish success/fail → share generated → `/sets/:id` view. Admin Making Layer panel shows `publishedSetsNonStaff` / 10 gate + 7d/30d conversion + top drop-off. Staff exclusion = `users.is_admin`. Identify 15s abort raised to 45s (OpenAI 40s). No public Maker Rate / volume claims.
