...
- [x] Pinned /sets covers (2026-09-26): `server/config/pinnedCovers.ts` maps `gameSetId` to an ordered card-id list, max 8, changed only by PR. Lists start empty. The public shelf and cover route serve only pins that pass `isBlockedCard`, the band guard, `eligibleDealFilter` plus sport match, and a baked masked JPEG. A failure drops that slot. Nothing auto-fills. Zero valid pins matches the hidden state. `SETS_COVERS_DISABLED` still forces that hidden state. Boot logs one `[PinnedCovers]` line per active integrated set. A short fan (1 to 8) spreads the real covers evenly: no empty slot and no cream or gray card. The set detail page with no covers shows the name, the year label, and Play, with no placeholder, gray box, card count, or coming soon.
