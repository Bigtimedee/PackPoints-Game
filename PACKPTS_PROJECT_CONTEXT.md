...
- `/sets` — play shelf for integrated sets already in PackPTS (`docs/SETS_POLISH.md`): eyebrow SETS, H1 “Sets”, sub “Play sets already in PackPTS.”, honest `{n} sets` from that list, Surface A cover crop or masked stack, integrated meta is `{n} cards` only (no `by Maker`, no date, no AUTHORED), blue **Play this set**, short-shelf banner when the list is below 10, empty state only at zero sets, Play Daily 5 (no Make a set). No search chrome, no play counts, no Maker Rate. Also in mobile nav ("Browse") and home game-modes grid.
