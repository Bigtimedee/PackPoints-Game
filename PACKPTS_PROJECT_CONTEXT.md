...
10. Route registration finishes, then production mounts the SPA catch-all (`serveStatic`) last. `markSchemaReady()` opens DB routes and logs `phase=routes_ready`. The early window handler then calls `next()` for every path. Boot logs also stamp `listen`, `pg_dump_start` / `pg_dump_end`, and `drizzle_push_start` / `drizzle_push_end` (`[Startup] phase=<name> ts=<iso>`). Missing `/assets/*` files log `[Static] 404 METHOD path` and do not print an ENOENT stack. `pg_dump` is killed after 90s and the push is skipped. `drizzle-kit push` is killed after 120s and the process exits 1. `storage.initialize()` is killed after 60s and the process exits 1. A normal storage error stays non-fatal.
...
The database has **144+ tables** defined in `shared/schema.ts` using Drizzle ORM. Production boot runs `drizzle-kit push --force` from `server/startup/bootSchema.ts` after the port is listening and before any DB-dependent route serves traffic. That push drops tables that are not in `shared/schema.ts`. `job_queue`, `promotions`, `user_attribution`, `creator_applications`, `partner_inquiries`, and `user_feedback` are declared there for that reason. `refresh_tokens`, `apns_tokens`, and `apple_transactions` were already declared. Below are the major domain groups with key tables.
