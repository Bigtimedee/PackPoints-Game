...
11. Background jobs start after routes are ready (on the fast path that overlaps the background schema push): risk pipeline, image validation (6h), card pool refresh (12h), session cleanup (1h), match cleanup (1h), redemption cleanup (1h). SIGTERM/SIGINT keep serving for 25s (Railway drain is 30s), then close idle keep-alives and the listener, run shutdown hooks (kill the schema push process group, stop the sidecar walk, stop jobs), close the pool, then exit.
