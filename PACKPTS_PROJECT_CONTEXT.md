...
- **Per-card name plate (2026-09-26, files are v4.6):** The bake runs name detection over the top plate, the bottom plate, and the full image, regardless of the set profile. The band goes on every plate where the surname is read. The profile is the prior only when no surname is read and that edge is not contradicted by letter rows on the other edge. A contradicted plate with no surname read is `name_plate_unresolved` and the card is excluded, except a TOP_PLATE profile whose band is already in `TOP_PLATE_PLACEMENT_CONTRACT` (`1987-topps-football` top 24%, `fleer-bball-top` top 18%). That miss paints the contract band, then runs `verifyNameVisibleOutsideMask`. A failed or skipped check still refuses. A pass writes `playable_cards.mask_fallback_state = fallback_pending_review`. The card is not dealable in Daily 5, solo, 1v1, Beat-me, replace, or covers until its id is a key of `MASK_FALLBACK_REVIEWED_CARD_IDS` in `server/config/maskFallbackReviewed.ts` (empty). `eligibleDealFilter` and `cardNotBlockedSql` both apply that predicate. `GET /api/qa/mask-fallbacks` lists those rows behind `COVER_QA_TOKEN` and `X-QA-Token`. The 1989 Fleer band itself is unchanged. Non-TOP_PLATE sets keep refusing. Jerseys are still not masked. The post-bake 4-letter surname check still excludes a readable surname outside the mask on every surface. `name_layout_verified` on `playable_cards` lets a subset card on a `quarantineSubsets` set back into deals after that check passes. 1987 Topps Football (`91cfdf3f-a620-4e73-adc8-22b8df221716`) is the flagged set. Subset text is read from `variant`, `description`, `number`, and `player`. There is no subset or cardType column.
