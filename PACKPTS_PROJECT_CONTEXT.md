...
**Last verified against codebase:** 2026-09-16 (post-submit full-card reveal: overlay-off + unmasked `/api/images/card/:id` after successful submit only; guessing stays v4.0 bake + overlay. Play Again #87; name masking v4.0 #86; Daily 5 overlay #85)
...
- **Post-submit full-card reveal (2026-09-16):** Dave’s rule: mask stays until a **successful** answer submit (correct or wrong), then the **full** card is shown; the next card starts masked. Guessing phase keeps `#86` bake (`/api/cards/:id/masked-image?v=v4.0`) plus the GameCard overlay. After ACK, parents swap `img src` via `resolvePlayCardSrc` to `/api/images/card/:id` and drop the overlay (`isRevealed`). Solo sets `isRevealed` in submit `onSuccess` only (not on Submit click). Daily 5 `onSuccess`. 1v1 `answerResult !== null`. 1v1 replace uses `maskedCardImageUrl`. WS `sanitizeMatchStateForClient` sends `gameSetId` so overlay profiles match the set. `GameCard` stays dumb (renders the URL it is given). Do not weaken v4.0 localization to fake a reveal. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`.
...
- [x] Post-submit full-card reveal (Dave, 2026-09-16): after a successful answer submit the overlay drops **and** `img src` swaps to `/api/images/card/:id`; guessing stays v4.0 bake + overlay. Solo `setIsRevealed(true)` is ACK-only. 1v1 replace uses `maskedCardImageUrl`. WS passes `gameSetId`. Helper `shared/playCardImage.ts`. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`. Do not regress #85 / #86 / #87.
