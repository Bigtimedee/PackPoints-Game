...
- **Share cards:** `GET /api/content-assets/latest`, `POST /api/content-assets/retry` — score-card PNG lookup + regenerate (`matchId` / `challengeId`) and maker-share PNG lookup + regenerate (`setId`). Files live on the Railway volume at `/app/data/masked-cards/generated/share/` (the non-root `packpts` user cannot write `/app/public`). Public URL prefix `/generated/share/` is mounted from that directory in production and from `public/generated/share` in local/CI. Inter TTFs ship in `server/contentFactory/assets/fonts/` and are outlined into SVG paths at generate time — Railway Alpine has no system fonts, so `<text font-family="sans-serif">` produced tofu on the live 1080 card. Maker share contract: `docs/MAKER_SHARE_CONTRACT.md` (1080 square, `I MADE THIS SET`, the published set’s masked cards — never stock vector fans — plus set name, mixtape note, honest `{N} cards`; no Maker Rate / DAU copy). Daily 5 Game Complete **Beat me** is a signed challenge token (not a PNG/caption): `POST /api/daily5/beat-me`, `GET /api/daily5/beat-me?challenge=` — see `docs/DAILY_BEAT_ME.md`. The 1080 card still ships as the share image (live `X/5`, optional real streak, SCORE_CARD_CONTRACT **§3b** `{MON} {D} · TODAY'S FIVE` + mini masked-strip, palette `#0b0f16` / `#F5C518` / `#22C55E` / `#F0F2F5` / `#8F96A3`, masked-P). Visual footer `packpts.com/daily`; real href is the token URL. Day key is America/Chicago via `shared/packptsDay.ts`.
...
- [x] Maker share v2 (2026-09-08): after Snap-to-Set / collab publish, default share art is a runtime 1080 PNG of that set’s masked cards + set name + mixtape note (`MAKER_SHARE_CARD`). Stock vector fans are not the default. No Maker Rate / DAU copy. Contract: `docs/MAKER_SHARE_CONTRACT.md`.
...
- `server/routes/collab.ts` (mounted in routes.ts): `POST /api/collab/create`, `GET /api/collab/:id`, `POST /api/collab/:id/join`, `/nominate`, `/approve` (can't approve own nomination), `/publish` (host only, ≥5 approved cards; sets `coCreatorUserId`; same maker-share PNG as `/api/sets/create`).
...
- `/make` — 3-step wizard (Upload → Review → Publish) + "Make it together" button that creates a collab session. After publish, default share preview is the runtime **I MADE THIS SET** PNG (that set’s masked cards + name + mixtape), not stock fan art.
...
After a user publishes via Snap-to-Set or collab, `onSetPublished` writes a 1080×1080 PNG (`content_assets.asset_type = MAKER_SHARE_CARD`, `source_event_id = maker_set_{setId}`) from the live set: up to five identified card photos with brand + gameplay name-band redaction, the published set name, the mixtape note, and the real playable-card count. Stock vector fans are not used. Public Maker Rate / volume claims stay gated until ≥10 non-staff published sets — this card is the individual’s own set, not a platform brag. Locked Design contract: `docs/MAKER_SHARE_CONTRACT.md`.
