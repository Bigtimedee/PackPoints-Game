...
- [x] Dealable-card QA sweep (2026-09-26): the same token opens `GET /api/qa/sets` and `GET /api/qa/sets/:setId/dealable-cards`. Unset, blank, missing, or wrong `X-QA-Token` is 404. A token in the query string is ignored. The set list is active integrated sets (`is_active`, not user-created) with a total from `eligibleDealFilter`. The card page is that same filter, ordered by `number` then `cardId`, `offset` default 0, `limit` default 200 and max 500. Each card is `{ cardId, player, number, variant, bandPlacement, baked }`. No new table.
- [x] Mask-bake refusal log (2026-09-26): every v4.6 bake that refuses (coverage or enforced band) inserts `mask_bake_refusals` with card id, set id, reason, layout class, profile source, the expected profile plate in source pixels, OCR boxes (text and confidence), candidate plates and why each failed, paint regions, mask version, timestamp, and the upright source bytes. `GET /api/qa/rejected-cards` lists those rows (no image bytes). `debug.png` draws the expected plate, candidates, and OCR boxes. `source` is the raw upright scan. `attempt.png` is the dry-run paint of the stored regions. Same token gate and no-store headers. Those images are not written to the masked-card cache and are not linked from player routes. `name_plate_unresolved` stays fail-closed. Card pool refresh skips a card when `cardPoolMayRestore` is false, using `MASK_BAKE_BLOCK_REASONS` (also inside `eligibleDealFilter`) plus the current fail sidecar.
