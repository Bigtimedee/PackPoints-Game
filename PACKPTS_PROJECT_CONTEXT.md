...
7. If unauthenticated, a signup modal prompts the user to save their score. After the first finished guest round the modal is a soft prompt (secondary **Play one more round**). After the hard gate there is no skip and no Play Again. An in-progress round is not interrupted.
...
- **Allowance:** Soft prompt after 1 completed Daily 5, `/sets`, or home solo round (same CT day) — they may start one more round. Hard stop when `max(gamesCompleted, gamesStarted) >= 2`, or when `lastPlayDay` is an earlier CT day and they have used a start. Abandoned starts count (`games_started` increments only if it is still `< 2`). In-progress Daily 5 resume and an already-started solo `/api/game/next` are not new starts.
- **Escrow:** Points sit on `anon_players` (`anon_game_credits` id `play:<sessionId>` or `daily5:<runId>`). Register, local login, and WorkOS callback call `claimAnonForUser`, which `walletService.earn`s `anon_escrow:<anonPlayerId>` and `updateUserStats`. Fast Daily 5 finishes (&lt;15s) escrow 0 and still count as a round. Guest scores are not on the public Daily 5 leaderboard until claim.
...
- **Server JPEG:** `maskingService.ts` + `maskCardImage.ts` bake solid RGB name-region overlays (`#0a0e16`, no alpha) into `/api/cards/:id/masked-image?v=v4.3`. Cache: `card_image_mask_cache.maskVersion` + filename `{cardId}_v4.3.jpg` on the Railway volume. Warm hits peek that file and skip DB/OCR (`X-Mask-Cache: hit`). HTTP `ETag` / `X-Mask-Version` follow the bake id; `Cache-Control` is `public, max-age=86400, stale-while-revalidate=604800`. The route ignores `?v=` for file lookup — gameplay URLs must bump `CURRENT_MASK_VERSION` so CDNs fetch a new object. Rebuild: `POST /api/admin/masks/rebuild` `{ "all": true }` (or `{ "cardIds": ["…"] }`) — `docs/MASK_CACHE_REBUILD.md`. Overlay stays set-level; per-card slab labels are covered by the baked JPEG.
...
- **Post-submit full-card reveal (2026-09-16):** Dave’s rule: mask stays until a **successful** answer submit (correct or wrong), then the **full** card is shown; the next card starts masked. Guessing phase keeps the current bake (`/api/cards/:id/masked-image?v=v4.3`) plus the GameCard overlay. After ACK, parents swap `img src` via `resolvePlayCardSrc` to `/api/images/card/:id` and drop the overlay (`isRevealed`). Solo sets `isRevealed` in submit `onSuccess` only (not on Submit click). Daily 5 `onSuccess`. 1v1 `answerResult !== null`. 1v1 replace uses `maskedCardImageUrl`. WS `sanitizeMatchStateForClient` sends `gameSetId` so overlay profiles match the set. `GameCard` stays dumb (renders the URL it is given). Do not weaken localization to fake a reveal. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`.
...
**v4.2 geometry:** The band is **not** hardcoded to `bottom: 0, height: 46%`. It follows overlay regions from mask config (top plate for 1989 Fleer Basketball, bottom plaque for 1987 Topps, OCR boxes when present). PSA-slab bakes add a top cert-label region on the JPEG; the set overlay can stay plaque-only. If config is empty, `overlayMaskRegions()` falls back to `DEFAULT_MASK_REGIONS`. Never remove the always-on render; never re-hardcode a bottom-only band.
...
- [x] Post-submit full-card reveal (Dave, 2026-09-16): after a successful answer submit the overlay drops **and** `img src` swaps to `/api/images/card/:id`; guessing stays the current bake + overlay. Solo `setIsRevealed(true)` is ACK-only. 1v1 replace uses `maskedCardImageUrl`. WS passes `gameSetId`. Helper `shared/playCardImage.ts`. Audit: `docs/audits/MASK_REVEAL_AFTER_SUBMIT_2026-09-16.md`. Do not regress #85 / #86 / #87.
...
**Product lock (2026-09-08):** users NEVER create cards. PackPTS plays sets already integrated into the product. The Snap-to-Set `/make` wizard and collab publish path stay in the repo (not deleted) but are **darked for non-staff**: no public nav, home CTA, `/sets` “Make a set”, set-page “Make another”, or profile publish funnel. Non-admin visitors to `/make` are redirected to `/sets`. Staff (`users.is_admin`) may still reach `/make` by URL for ops. Identify + publish APIs require admin.
...
- MAKE_FLOW (locked wiring): auth-before-upload; two CTAs only (Take photo / Choose from library); sequential identify; publish → Surface A share kit. Empty `/make` CTAs stay clickable during auth hydration (intent queued). Failed identify offers Retry + New photo. Name + mixtape both required (no “Untitled set” placeholder that looks pre-filled).
...
- `/make` — **staff-only** (code retained, public path dark). Non-admin / guest is redirected to `/sets` (`MakeRoute` + `canAccessMake`). Staff still get the 3-step wizard (Upload → Review → Publish) per MAKE_FLOW, including staff QA `?qaIdentifyFail=1` / `?qa=identify-fail`. No public nav or home tile. Contracts: `docs/design/EMPTY_STATE.md`, `docs/design/IDENTIFY_RETRY.md`, `docs/design/ENG_HANDOFF.md`. Funnel: `event_log` via `/api/make/event`.
