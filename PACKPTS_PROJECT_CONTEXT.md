...
- `/sets` — play shelf for integrated sets already in PackPTS (`docs/SETS_POLISH.md`): eyebrow SETS, H1 “Sets”, sub “Play sets already in PackPTS.”, honest `{n} sets` from that list, Surface A cover crop or a fanned stack of baked masked cover URLs (cream placeholder when no sidecar is ready) whose plaque is a solid bar with the gold seam and no label text, integrated meta is `{n} cards` only (no `by Maker`, no date, no AUTHORED), blue **Play this set**, no short-shelf banner, empty state only at zero sets, Play Daily 5 (no Make a set). A stored brand missing from the set name is inserted into the heading (`2024 Basketball` + Topps → `2024 Topps Basketball`). No search chrome, no play counts, no Maker Rate, no playable-card vanity total. Also in mobile nav ("Browse") and home game-modes grid.
