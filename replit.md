...
Video Factory (`server/videoFactory/`): Automated MP4 video generation system for TikTok content. Controlled by `VIDEO_FACTORY_ENABLED` env var (default: false). Optional TTS voiceover via `VIDEO_TTS_ENABLED` (uses OpenAI TTS API, model: tts-1, voice: nova). Core modules: `index.ts` (orchestrator), `ffmpegPath.ts` (FFmpeg binary resolution via ffmpeg-static), `render.ts` (FFmpeg renderer), `renderFormats.ts` (multi-template renderers), `validate.ts` (image validation/masking), `templates.ts` (template definitions), `tts.ts` (voiceover), `workerJob.ts` (background job). Videos are 1080x1920 (9:16), 12s duration, H.264 MP4 with blurred background, centered masked card image, and timed text overlays (hook/countdown/reveal/CTA). Image masking applies 18% top and 22% bottom gradient bands to hide player names. Placeholder images are rejected via pattern-based URL filtering and file size checks. Output stored in `public/generated/videos/{date}/{contentItemId}/`. Background job `render_tiktok_videos` scheduled at 13:35 UTC (after TikTok package generation). Admin UI has Render Video / Re-render / Download MP4 / Download Thumbnail buttons. API endpoints: `POST /api/admin/growth/queue/:id/render-video`, `POST /api/admin/growth/queue/bulk-render`, `GET /api/admin/growth/video-factory/config`. FFmpeg dependency: Uses `ffmpeg-static` npm package (bundled binary) to ensure availability in both development and autoscale deployment environments. The FFmpeg path is resolved in `ffmpegPath.ts` and shared across all render modules.

Instagram & Facebook manual posting (`server/services/growth/crossPostJobs.ts`): The `crosspost_to_ig_fb` job (scheduled at 13:40 UTC) generates Instagram and Facebook copies of TikTok content items with `postingMode: "MANUAL_QUEUE"`. These appear in the Admin Publishing Queue for manual copy-paste posting. No auto-posting to Instagram or Facebook — all IG/FB content is managed manually by admins via the Queue tab at `/admin/growth`. The `auto_post_ready_content` job only auto-posts to X/Twitter; IG/FB platforms are excluded.

DB tables: `growth_content_plans`, `growth_content_items`, `growth_job_runs`, `publishing_queue`. X/Twitter secrets: `TWITTER_API_KEY`, `TWITTER_API_SECRET`, `TWITTER_ACCESS_TOKEN`, `TWITTER_ACCESS_SECRET`.
